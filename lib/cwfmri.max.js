...
var pauseTransition = function pauseTransition() {
  ...
  try {
    ...
    var text2d = void 0;
    if (level <= 3) {
      this.taskObject.setCheckpoint(R.get.checkpoint_levelDone[level - 1]).promise.then(function () {
      ...
      var transitionText = function transitionText() {
        var timeLeft = Math.ceil((maxTransitionTime - stateManager.timeInMs + _startTransitionTime) / 1000, 10);
        if (timeLeft < 0) {
          return false;
        }
