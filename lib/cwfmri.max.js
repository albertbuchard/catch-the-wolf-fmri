...
var redrawAssets = function redrawAssets() {
  ...
  if (elements2D !== null) {
    ...
    wolfFrame = elements2D.wolfFaceSprite.spriteFrame;
    ...
    if (elements2D.tiles && elements2D.tiles.length && 'levelVisible' in elements2D.tiles[0]) {
      ...
      var options = {};
      if ((0, _experimentJs.hasConstructor)(_experimentJs.Array, elements2D.keysInfobox) && (0, _experimentJs.hasConstructor)(_experimentBabylonJs2.default.Rectangle2D, elements2D.keysInfobox[0]) && elements2D.keysInfobox[0].levelVisible) {
  ...
  /* --- Setup the loader --- */
  var loader = new _experimentJs.Loader({ parent: this.scene.initialCanvas, size: new _experimentBabylonJs2.default.Size(23, 23), border: _experimentBabylonJs2.default.Canvas2D.GetSolidColorBrush(new _experimentBabylonJs2.default.Color4(0.7, 0.7, 0.7, 1)), borderThickness: 1 });
  loader.value = loaderValue;
  loader.opacity = loaderOpacity;
  _lodash2.default.extend(loader.margin, { leftPixels: 0.5, rightPixels: 0, topPixels: 0, bottomPixels: 0.5 });
  ...
  var wolfFaceSprite = new _experimentBabylonJs2.default.Sprite2D(wolfFaceTexture, {
    ...
    marginAlignment: 'v: center, h: center',
    zOrder: 1,
    size: new _experimentBabylonJs2.default.Size(minSize * this.R.get.agent_scale, minSize * this.R.get.agent_scale / (400 / 550)), // getAspectRatio(wolfFaceTexture)),
    spriteSize: new _experimentBabylonJs2.default.Size(400, 550),
    spriteLocation: new _experimentBabylonJs2.default.Vector2(0, 0)
  });
  wolfFaceSprite.spriteFrame = wolfFrame;
...
var drawTiles = function drawTiles() {
  ...
  for (var i = 0; i < 4; i++) {
    if ((0, _experimentJs.hasConstructor)(_experimentBabylonJs2.default.Rectangle2D, elements2D.tiles[i])) {
      elements2D.tiles[i].dispose();
    }
    ...
    // }
    (0, _experimentJs.debugWarn)('NORMAL SIZE:', normalSize);
    ...
    } else if ((0, _experimentJs.hasConstructor)(_experimentJs.String, options.withKeys)) {
      if ((0, _experimentJs.hasConstructor)(_experimentBabylonJs2.default.Rectangle2D, elements2D.keysInfobox[i])) {
        elements2D.keysInfobox[i].dispose();
      }
...
var _experimentBoxes = __webpack_require__(4);
...
window.loadingPromise.then(function () {
  ...
  // Level 4 is 1, 3, 2, 4 wolf -- 0.75 -- 20% no go -- for rabbit position 1 with 0.38
  // 51 positions wolf + 50 positons rabbit + 51 positions wolf = 150 transitions -- ~ 7min * 3 + 3*6min pause
  ...
  taskObject.assetsToLoad = {
    ...
    wolfFace: '/assets/sprites/wolf/wolfFaceSprite.png',
    crossSheet: {
  ...
  taskObject.dataManager.authorize_manual_login = false;
...
var _globalFunctions = __webpack_require__(5);
...
function MainScene() {
  ...
  var surveyDataFields = ['level', 'similarTask', 'taskDifficulty', 'movementWolf', 'boredom', 'dependance', 'repeatsSequence'];
...
var startLevel = function startLevel() {
  ...
  stateManager.set('positionData', {
    indexOnTransitionSequence: -1,
    indexOnPredictionSequence: -1,
    fullSequenceIndex: -1 // taskObject.parameters.levels[options.data.level - 1].sequenceObject.fullSequence.length - 3, // TODO PUT IT BACK -1,
  });
...
var pauseTransition = function pauseTransition() {
  ...
  try {
    ...
    var addBlock = typeof sequenceObject.block[positionData.fullSequenceIndex - 1] !== 'undefined' ? sequenceObject.block[positionData.fullSequenceIndex - 1] : 0;

    var askSurvey = function askSurvey() {
      var surveyData = {
        level: level + addBlock,
        similarTask: null,
        taskDifficulty: null,
        movementWolf: null,
        boredom: null,
        dependance: null,
        repeatsSequence: null,
        comments: null,
        belongsTo: ['surveyData']
      };

      var fields = {
        taskDifficulty: {
          type: 'select', // field type: input, select, textaera, slider, radio
          constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
          authorizedValues: ['Too easy', 'Easy', 'Good difficulty', 'A bit hard', 'Too hard'], // athorized values
          parent: null,
          title: 'How hard is the task ?'
        },
        movementWolf: {
          type: 'radio', // field type: input, select, textaera, slider, radio
          constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
          authorizedValues: ['Yes', 'No'], // athorized values
          parent: null,
          title: 'Do you think the positions of the animal are totally unpredictable ?'
        },
        boredom: {
          type: 'slider', // field type: input, select, textaera, slider, radio
          constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
          authorizedValues: [0, 100, 1], // athorized values
          parent: null,
          title: 'How bored are you from 0 (enjoying the task) to 100 (extremely bored) ?'
        }
      ...
      if (level === 1) {
        fields.similarTask = {
          ...
          authorizedValues: ['Yes', 'No'], // athorized values
          parent: null,
          title: 'Have you ever done an experiment in the past that seem similar to this one ?'
        };
      }

      var options = { fields: fields, title: 'Debriefing', format: 'topCentralSmall' };

      return new _experimentBoxes.SmartForm(options).promise.then(function (results) {
        _lodash2.default.map(results, function (e, k) {
          surveyData[k] = e.value;
        });

        if (results.movementWolf.value === 'No') {
          var _fields = {
            dependance: {
              type: 'radio', // field type: input, select, textaera, slider, radio
              constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
              authorizedValues: ['Yes', 'No', 'I do not know'], // athorized values
              parent: null,
              title: 'The next position of the animal depends only on its last position ?'
            },
            repeatsSequence: {
              type: 'select', // field type: input, select, textaera, slider, radio
              constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
              authorizedValues: ['I do not know', 'Does not repeat', 'Repeats every 2 positions', 'Repeats every 4 positions', 'Repeats every 6 positions', 'Repeats every 8 positions', 'Repeats every 10 positions', 'Repeats every 12 positions', 'Repeats every 16 positions', 'Takes more than 20 positions'], // athorized values
              parent: null,
              title: 'Does the animal repeats a similar pattern ?'
            },
            comments: {
              type: 'textaera', // field type: input, select, textaera, slider, radio
              constraints: '', // list of constraints that will be automatically verified: mandatory; alpha; numeric; length:XX; contains:a,b,@,.;
              authorizedValues: null, // athorized values
              parent: null,
          ...
          var _options = { fields: _fields, title: 'Debriefing', format: 'topCentralSmall' };

          return new _experimentBoxes.SmartForm(_options).promise;
        }
        ...
        return stateManager.storeData(surveyData);
      });
    };
    ...
    var text2d = void 0;
    if (level <= 3) {
      this.taskObject.setCheckpoint(R.get.checkpoint_levelDone[level - 1]).promise.then(askSurvey).then(function () {
        ...
        state.tooltipPause = stateManager.tooltip({ position: centerPosition.add(new _experimentBabylonJs2.default.Vector2(0, _this.taskObject.renderSize.height / 2 - 100)), text: transitionText(), duration: promise });
      ...
      askSurvey().then(function () {
        positionData.fullSequenceIndex -= 1;
        var blockDone = sequenceObject.block[positionData.fullSequenceIndex];
        var startTransitionTime = stateManager.timeInMs;
        var maxTransitionTime = 15 * 60 * 1000;
        var promise = stateManager.resolveOnKey({ except: null });
        var shouldtic = true;
        ...
        var transitionText = function transitionText() {
          ...
          return R.get.finishedLevelTransition.replace('{n}', (0, _experimentJs.String)(timeLeft)).replace('{l}', level + blockDone);
        };
...
var startRunning = function startRunning() {
  ...
  var promise = stateManager.resolveOnKey({ except: null });
  stateManager.tooltip({ position: centerPosition.add(new _experimentBabylonJs2.default.Vector2(0, this.taskObject.renderSize.height / 2 - 100)), text: 'You are about to start the level\nHit a key to when ready', duration: promise });
  ...
  try {
    if ((0, _experimentJs.hasConstructor)(Object, state.currentTrialData) && stateManager.get('subject_id')) {
      ...
      dataToStore = {
        ...
        dominant_structure: typeof levelObject.dominantPositionRandom !== 'undefined' && blockType === 'random' ? levelObject.dominantPositionRandom : levelObject.dominantStructure,
...
var goToNextPosition = function goToNextPosition() {
  ...
  if (levelObject.level === 4) {
    elements2D.loader.value = 100 * (index - sequenceObject.block[index] * 50) / 50;
  } else {
...
var pageOrderTutorialFMRI = ['fmriIntro', 'fmriRabbit', 'fmriNoPrediction', 'lockedCross'];
